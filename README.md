...
## Run the project ##
Due to the enormous size of climate datasets and huge computation power requirement, we recommend to run the project on supercomputing cluster. The folder ***Run_project*** contains python and bash scripts which can be run sequentially to reproduce the results. Please read the **Running_instruction.txt**, which can be found in the folder, at first.
